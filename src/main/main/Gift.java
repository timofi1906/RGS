...
public class Gift{
    ...
    private void FindGifts(String Url) {
        ...
        try {
            ...
            for (Element e : names){
                if (e.attr("title").length() != 0 && !e.attr("title").contains("More like this")) {
                    name.put(e.attr("title"),e.attr("href"));
                }
            }
            List<String> keysAsArray = new ArrayList<>(name.keySet());
            int var = random.nextInt(keysAsArray.size());

            String n = name.get(keysAsArray.get(var));
            gifts.put(keysAsArray.get(var),n);
    ...
    public void BirthDayGreetings() throws IOException {
        ...
        System.out.printf("Gift for %s%n", FirstName + " " + LastName);
        ...
        System.out.println("Do yo want change message?(Y|N):");
